  ...
  - name: Jane Doe
    affiliations:
      - name: Guardant Health
        department: AI and Translational Medicine (ATM)
    email: jane.doe@guardanthealth.com
  - name: John Doe
    affiliations:
      - name: Guardant Health
        department: Bioinformatics
    email: john.doe@guardanthealth.com
date: "`r Sys.Date()`"
...
logo1: images/GH_logo.png  # logo on left side of title page
logo2: images/GH_secondary_logo.png  # logo on right side of title page
...
format: 
  pdf:
    ...
    # Comment out cover-bg-image line if no cover image is desired
    # When no cover image is provided, the title will appear near the top with default 3cm spacing
    cover-bg-image: "images/GH_cover_lab.png"
    cover-page-color: "FFFFFF"  # provide hex color code
    cover-text-color: "4A5054"  # Guardant Health Data Gray
...
knitr::write_bib(
  x = c(.packages(), 'rmarkdown', 'GHformats',
...
The default font for this template is 'Helvetica' but can be replaced with the Guardant Health's own font style 'HelveticaNeue'.You can choose the font in the template when running the function, e.g., `GHformats::create_quarto_doc(dirname = "your_pdf_report, template = "pdf_report", font = "HelveticaNeue")`.  
...
All analyses were performed using the statistical software R (version `r paste(R.Version()$major, R.Version()$minor, sep = ".")`) [@R-base]. This thesis, including tables, was generated using the R packages 'rmarkdown' (version `r packageVersion("rmarkdown")`) [@R-rmarkdown], 'GHformats' (version `r packageVersion("GHformats")`) [@R-GHformats], and 'knitr' (version `r packageVersion("knitr")`) [@R-knitr].
