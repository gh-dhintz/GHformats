...
params:
  # replace path to images or remove or comment out lines (one has to be kept in!)
  # The title will be positioned near the top of first page when there is no cover image and below the cover image if there is one
  cover: images/GH_cover_lab.png
...
header-includes: 
  ...
  - \renewcommand{\nocovertitlespace}{-0cm} # resets title placement. DON'T COMMENT OUT. You can alter it for higher or lower placement; ie., \renewcommand{\nocovertitlespace}{-1cm} or \renewcommand{\nocovertitlespace}{1cm} etc
output: 
  GHformats::pdf_report:
    font: "HelveticaNeue" # alternative: "Helvetica"  
---
...
knitr::write_bib(
  x = c(.packages(), 'bookdown', 'rmarkdown', 'GHformats',
    # Now add all packages that are loaded above:
...
* `output`: The nested fields for the output field are based on the arguments of the output function. Since `GHformats::pdf_report` is based on `rmarkdown::pdf_document`, see its help page for more options. Current default settings are
    * `toc = TRUE`
    * `toc_depth = 5`
...
You can also use "Helvetica" instead of the default "HelveticaNeue". To use another font entirely, use the setting "other" and replace the 'font_XXX.ttf' files in the working directory with your own files. Please note that you have to name these files exactly as the template font files.
...
Table \ref{tab:rmd_tab} is an R Markdown table including a caption (note: the table number is automatically assigned) and label for cross-referencing: 
...
All analyses were performed using the statistical software R (version `r paste(R.Version()$major, R.Version()$minor, sep = ".")`) [@R-base]. This report, including tables and figures, was generated using the packages 'rmarkdown' (version `r packageVersion("rmarkdown")`) [@R-rmarkdown], 'bookdown' (version `r packageVersion("bookdown")`) [@R-bookdown], 'GHformats' (version `r packageVersion("GHformats")`) [@R-GHformats], 'knitr' (version `r packageVersion("knitr")`) [@R-knitr], 'kableExtra' (version `r packageVersion("kableExtra")`) [@R-kableExtra], 'xtable' (version `r packageVersion("xtable")`) [@R-xtable], and 'tidyverse' (version `r packageVersion("tidyverse")`) [@R-tidyverse.]
